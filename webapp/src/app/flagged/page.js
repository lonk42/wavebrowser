...
function Flagged() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/flagged", { cache: "no-store" });
  ...
  // Toggle a recording's bookmark in place (this list is keyed on `interesting`,
  // not `bookmarked`, so starring shouldn't remove the card). Optimistic; reverts
  // on failure.
  const handleToggleBookmark = (id, next) => {
    setItems((prev) =>
      prev.map((it) => (it._id === id ? { ...it, bookmarked: next } : it))
    );
    fetch("/api/bookmark", {
      method: "POST",
      ...
      <header className="sticky top-0 z-30 border-b border-line bg-bg/80 backdrop-blur-xl">
        ...
        <div className="relative mx-auto flex max-w-5xl items-center justify-between gap-4 px-4 pb-4 pt-5 sm:px-6">
          <div className="flex items-center gap-3">
            <span className="grid size-9 place-items-center rounded-lg border border-ai/30 bg-ai-soft text-ai">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center gap-3 py-28 text-center">
            ...
            <p className="max-w-xs text-sm text-muted">
              The flagger promotes the most interesting transmissions each day; they show up here.
            </p>
          </div>
        ) : (
